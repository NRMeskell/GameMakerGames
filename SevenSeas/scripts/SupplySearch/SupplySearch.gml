function SearchSupplies(){
	...
	if myPortType = 0
		...
		text = getItemText(loot, ["", "", "", "round stones", choose("branches", "scraps"), "fish"]) + " found scattered around the rocks!"
		}
	    ...
	    {
		title = "old wreckage"
		loot = GetRandomLoot(max(0, 3  + irandom_range(-2,-1)), "supplies", [2,3,4,5])
	    ...
	    {
		title = "barren rocks"
		loot = GetRandomLoot(max(0, 3  + irandom_range(-2,-1)), "supplies", [0,1,2,2,2,3])
