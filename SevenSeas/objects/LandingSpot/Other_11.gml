...
image_index = myIndex
if visited or (image_index = 1 and myIslandType != global.seaNames[4])
	sprite_index = StopSpr
else
	sprite_index = UnknownStopSpr

if released and MapCreator.drawMiniHeight > 0 and 
  (max(x, checkX) > camera_get_view_x(view_camera[1]) - 16 or min(x, checkX) < camera_get_view_x(1) + camera_get_view_width(view_camera[1])) + 16 and
  (max(y, checkY) > camera_get_view_y(view_camera[1]) - 32 or min(y, checkY) < camera_get_view_y(1) + camera_get_view_height(view_camera[1]) + 32) {
	visible = true
    dir = point_direction(x, y, checkX, checkY)
	
	// Draw landing spot above image
	if checkY < y{
