...
if playerTurn and __view_get( e__VW.XView, 0 ) = 0
    ...
    if !instance_exists(Event){
        for(var i=0; i<ds_list_size(possibleEnemyActions); i++){
			...
			if point_in_rectangle(mouse_x, mouse_y, drawOptionsX-20, drawOptionsY + i*drawOptionsDis - 20, drawOptionsX+20, drawOptionsY + i*drawOptionsDis + 20){
				...
				draw_sprite(PlayerAttackFrameSpr, 0, drawOptionsX, drawOptionsY + i*drawOptionsDis)
				draw_sprite(PlayerAttackActionsSpr, showAction.spriteNumber, drawOptionsX, drawOptionsY + i*drawOptionsDis)
			}
		}
    }
