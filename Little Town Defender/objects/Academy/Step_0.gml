...
if mouse_check_button_pressed(mb_left)
    ...
    if Created = true
    if global.WorkerVic >= 30
    if global.WorkerStage = 0
    if point_in_rectangle(mouse_x, mouse_y, ButtonLeft1, ButtonTop2, ButtonRight1, ButtonBottom2) //Top
    if global.Gold >= ArmyCost
    if global.Trees >= ArmyWood
    if global.Rocks >= ArmyStone
        {   
        with Enemy
            Damage = 0
        
        global.Gold -= ArmyCost
        global.Trees -= ArmyWood
        global.Rocks -= ArmyStone
