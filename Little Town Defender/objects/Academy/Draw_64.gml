...
if Selected = true
    ...
    draw_text(__view_get( e__VW.WView, 0 ) - (sprite_get_width(MenuArea)*MenuSize)/2 - Edge, __view_get( e__VW.HView, 0 ) - (sprite_get_height(MenuArea)*MenuSize) - Edge + CaptionPlace, string_hash_to_newline("Academy"))
    ...
    draw_text(__view_get( e__VW.WView, 0 ) - (sprite_get_width(MenuArea)*MenuSize)/2 - Edge, __view_get( e__VW.HView, 0 ) - (sprite_get_height(MenuArea)*MenuSize) - Edge + UpgradePlace + 4/2*Distance, string_hash_to_newline("Trains Workers#Into Army")) 
        
    draw_set_valign(fa_center)
    if Level < 2
        draw_text(__view_get( e__VW.WView, 0 ) - (sprite_get_width(MenuArea)*MenuSize)/2 - Edge, __view_get( e__VW.HView, 0 ) - (sprite_get_height(MenuArea)*MenuSize)* 2/5 - Edge + 2, string_hash_to_newline("Train")) 
...
if point_in_rectangle(mouse_x, mouse_y, ButtonLeft1, ButtonTop2, ButtonRight1, ButtonBottom2)
    ...
    draw_set_valign(fa_center)
    draw_sprite_ext(MenuArea, 0, mouse_x, mouse_y, 1.1, 0.38, 0, c_white, 1)

    draw_text(mouse_x + sprite_get_width(MenuArea) * 0.25 - 10, mouse_y + 1*DistanceDraw, string_hash_to_newline("Must Have"))
    draw_text(mouse_x + sprite_get_width(MenuArea) * 0.25 - 10, mouse_y + 2*DistanceDraw,  string_hash_to_newline("30 Worker"))
    draw_text(mouse_x + sprite_get_width(MenuArea) * 0.25 - 10, mouse_y + 3*DistanceDraw, string_hash_to_newline("Levels"))
