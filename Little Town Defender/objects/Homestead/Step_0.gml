...
if HasFood = 100
    {
    image_index = 4
    if instance_exists(Worker) and Created = true
        {
        for(i = 0; i <= instance_number(Worker); i ++)
            with instance_find(Worker,i)
                {
                if distance_to_object(other) < other.FindRange and (sprite_index = WorkerSpr or sprite_index = WorkerSpr2)
                    {
                    with Worker
                        FoodReady = false
                        
                    if Target.object_index != Homestead
                    ...
                    Working = false
                    FoodReady = true     
                    other.FindRange = distance_to_object(other)
                    }
                }        
        }
    }
...
CloseWorker = instance_nearest(x,y,Worker)
if point_distance(CloseWorker.x,CloseWorker.y,x,y) < 3 and HasFood == 100
    {
    HasFood = 0
    CloseWorker.Level += 0.5
    CloseWorker.FoodReady = false
    CloseWorker.Target = CloseWorker.MyHouse
    }
