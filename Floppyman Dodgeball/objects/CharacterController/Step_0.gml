...
if jumpPressed and !stunned and !slipped and !golfHitting and (standing) and alarm[1] < 1
	...
	with body
		physics_apply_impulse(x,y,0,-1.3/(myCharacter.wet+1)*sqrt(global.timeDiff))
...
if handBall[0] == 10
    {
    with hand1
        physics_apply_force(x, y, 0, -other.balloonForce*(global.timeDiff)*(1+myCharacter.handSuper[0]))
    
	if handSuper[0] > 0
		handSuper[0] += 0.00034
	if handSuper[0] > 0.75{
		with instance_create(hand1.x, hand1.y, Balloon)
			event_user(0)
		handBall[0] = -1
		handSuper[0] = false
		throwing = false
...
if handBall[1] == 10
    {
    with hand2
        physics_apply_force(x, y, 0, -other.balloonForce*(global.timeDiff)*(1+myCharacter.handSuper[1]))
    
	if handSuper[1] > 0
		handSuper[1] += 0.00034
	if handSuper[1] > 0.75{
		with instance_create(hand2.x, hand2.y, Balloon)
			event_user(0)
		handBall[1] = -1
		handSuper[1] = false
		throwing = false
...
if global.gameMode == "dodgeball" and !is_undefined(body){
    if !stunned and !slipped
        ...
        if instance_exists(DodgeBallParent)
            ...
            if point_distance(body.x, body.y+20, grabBall.x, grabBall.y) < grabDistance and grabBall.phy_speed < grabSpeed and grabBall.image_index != 7 and grabBall.alarm[0] < 1
                {
                if handBall[pickHand] == -1
                    {
					handSuper[pickHand] = grabBall.super
        ...
        if throwReleased and throwing = true and sticky = false
            ...
            with instance_create(throwx, body.y, ballThrowType[handBall[throwHand]])
                {
                thrower = other.id
				if object_index == SuperBall
					super = false
				else
					super = other.handSuper[other.throwHand]
                ...
                if image_index = 5
                    {
                    alarm[0] = room_speed * 5 *(1+super)/global.timeDiff
                    event_user(0)
                    }
                }
            if throwHand = 0
            ...
            if (ballThrowType[handBall[throwHand]] == SuperBall) and (handSuper[throwHand] > 1){
				handSuper[throwHand] -= 1
			} else{
				handSuper[throwHand] = false
